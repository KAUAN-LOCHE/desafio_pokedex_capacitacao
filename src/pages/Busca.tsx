...
import httpClient, { PokemonDetails } from "../api/http-client";
import {
  Radar,
  RadarChart,
  PolarGrid,
...
export default function Busca() {
  ...
  const handleSearch = async (e: React.FormEvent) => {
    ...
    try {
      ...
      const data = await httpClient.get<PokemonDetails>(`pokemon/${searchTerm.toLowerCase().trim()}`);
  ...
  return (
    <section className="content">
      ...
      <form onSubmit={handleSearch} style={{ display: 'flex', gap: '10px', marginBottom: '20px', width: '100%', maxWidth: '500px' }}>
        <input 
          type="text" 
          value={searchTerm} 
